"""Codex Agent Adapter implementation of IUniversalAgentAdapter (Block 26)."""
...
class CodexAdapter(IUniversalAgentAdapter):
    """Universal agent adapter for OpenAI Codex CLI and runtime (Block 26).

    Translates between the INFUSE universal agent contracts and Codex-specific
    process, tool execution, JSONL streaming, and control mechanics.
    """

    def __init__(
        self,
        agent_id: str = "codex_adapter",
        agent_name: str = "codex",
        version: str = "1.0.0",
        config: Optional[CodexAdapterConfig] = None,
        transport: Optional[ICodexTransport] = None,
        event_bus: Optional[IEventBus] = None,
    ) -> None:
        self._lock = threading.RLock()
        self._bus = event_bus
        self.config = config or CodexAdapterConfig()
        self.transport = transport or CodexSubprocessTransport(
        ...
        self._capability = AgentCapability(
            agent_name=agent_name,
            supported_protocols=["cli", "stdio", "jsonl", "json"],
            ...
            metadata={"vendor": "OpenAI Codex CLI Runtime"},
        )
    ...
    def execute_step(self, request: AgentStepRequest) -> AgentStepResponse:
        ...
        except CodexAdapterError as e:
            raise AgentExecutionError(str(e)) from e
        except Exception as e:
            raise AgentExecutionError(f"Unexpected Codex execution failure: {e}") from e
        ...
        if output.parsed_json:
            ...
            content = (
                ...
                or output.parsed_json.get("message")
                or output.stdout
            )
        ...
        resp = AgentStepResponse(
            ...
            metadata={
                ...
                "events_count": len(output.events_jsonl),
        ...
        if self._bus is not None:
            try:
                ...
                ev = ExecutionEvent(
                    event_id=f"step_ev_codex_{exec_id}_{request.step_index}_{int(now.timestamp()*1000)}",
                    execution_id=exec_id,
                ...
                # 2. Tool Activity events if observable from output or jsonl events
                tool_list = []
                if output.parsed_json and "tools" in output.parsed_json and isinstance(output.parsed_json["tools"], list):
                    tool_list.extend(output.parsed_json["tools"])
                elif output.parsed_json and "tool_calls" in output.parsed_json and isinstance(output.parsed_json["tool_calls"], list):
                    tool_list.extend(output.parsed_json["tool_calls"])

                for item in output.events_jsonl:
                    if item.get("type") == "tool_call" or "tool" in item:
                ...
                for idx, tool in enumerate(tool_list):
                    ...
                    call_ev = ExecutionEvent(
                        event_id=f"tool_called_codex_{exec_id}_{request.step_index}_{idx}",
                        execution_id=exec_id,
                        type=EventType.TOOL_CALLED,
                        source=EventSource.AGENT,
                        sequence=session.step_count,
                        timestamp=now,
                        payload={"tool_name": t_name, "arguments": tool.get("args") or tool.get("parameters") or {}},
                    )
                    self._bus.publish(call_ev)

                    comp_ev = ExecutionEvent(
                        event_id=f"tool_comp_codex_{exec_id}_{request.step_index}_{idx}",
                        execution_id=exec_id,
                ...
                # 3. Web Activity events if observable from output or jsonl events
                web_list = []
                if output.parsed_json and "web_requests" in output.parsed_json and isinstance(output.parsed_json["web_requests"], list):
                    web_list.extend(output.parsed_json["web_requests"])

                for item in output.events_jsonl:
                    if item.get("type") == "web_request" or "web_search" in item:
                        web_list.append(item)

                for idx, web in enumerate(web_list):
                    w_url = str(web.get("url") or f"http://codex.internal/{idx}")
                    req_ev = ExecutionEvent(
                        event_id=f"web_req_codex_{exec_id}_{request.step_index}_{idx}",
                        execution_id=exec_id,
                    ...
                    resp_ev = ExecutionEvent(
                        event_id=f"web_resp_codex_{exec_id}_{request.step_index}_{idx}",
                        execution_id=exec_id,
    ...
    def execute_control(self, operation: ControlOperation) -> ControlResult:
        ...
        if not is_supp:
            return ControlResult(
                ...
                message=f"Codex adapter does not support control action '{action_name}'.",
                executed_at=now,
        ...
        try:
            ...
            return ControlResult(
                ...
                message=f"Codex adapter successfully applied action '{action_name}'.",
                executed_at=now,
                metadata={"agent_id": self._identity.agent_id, "is_supported": True},
            )
        except Exception as e:
            redacted_err = redact_codex_secrets(str(e))
            return ControlResult(
                operation_id=operation.operation_id,
                execution_id=exec_id,
                action=gov_action,
                status=ControlStatus.FAILED,
                message=f"Codex adapter failed to execute control action '{action_name}': {redacted_err}",
                executed_at=now,
    ...
    def normalize_error(self, raw_error: Any, execution_id: Optional[str] = None) -> AgentErrorRecord:
        if isinstance(raw_error, Exception):
            ...
            code = str(raw_error.get("error_code") or raw_error.get("code") or "CODEX_ERROR")
            msg = str(raw_error.get("message") or raw_error.get("error") or "Unknown Codex error")
        else:
            code = "CODEX_ERROR"
            msg = str(raw_error)
        ...
        return AgentErrorRecord(
            error_code=code,
            message=redact_codex_secrets(msg),
