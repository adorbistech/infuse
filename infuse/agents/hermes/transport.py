"""Transport abstraction layer for Hermes CLI and process management (Block 27)."""
...
class IHermesTransport(ABC):
    """Abstract transport boundary between HermesAdapter and Hermes runtime."""

    @abstractmethod
    def is_available(self) -> bool:
        """Check whether the underlying Hermes runtime is available."""
        pass
    ...
    def execute(
        ...
        """Execute a Hermes CLI command with arguments."""
        pass
...
class HermesSubprocessTransport(IHermesTransport):
    """Subprocess-based transport communicating with local Hermes CLI."""
    ...
    def execute(
        ...
        cli = self._cli_path or self._discover_cli()
        if not cli:
            raise HermesCLINotFoundError("Hermes executable ('hermes') not found on system PATH.")
        ...
        try:
            ...
            try:
                ...
                raise HermesTimeoutError(
                    f"Hermes process timed out after {exec_timeout} seconds for execution '{execution_id}'."
                )
            ...
            if proc.returncode != 0:
                redacted_err = redact_hermes_secrets(stderr)
                raise HermesProcessError(
                    f"Hermes process exited with return code {proc.returncode}: {redacted_err}"
                )

            return HermesExecutionOutput(
                stdout=stdout or "",
                stderr=stderr or "",
...
class HermesReferenceTransport(IHermesTransport):
    """Deterministic reference/mock transport for hermetic testing of Hermes adapter."""
    ...
    def __init__(self) -> None:
        ...
        self.custom_output: Optional[HermesExecutionOutput] = None
    ...
    def execute(
        ...
        with self._lock:
            ...
            if self.should_raise_not_found:
                raise HermesCLINotFoundError("Hermes executable not found.")

            if self.should_timeout:
                raise HermesTimeoutError("Hermes reference execution timed out.")

            if self.should_fail:
                raise HermesProcessError("Hermes reference process exited with error.")
            ...
            return HermesExecutionOutput(
                stdout=f"Hermes agent response for: {prompt_str[:40]}",
                stderr="",
                return_code=0,
                parsed_json={
                    "result": f"Hermes agent response for: {prompt_str[:40]}",
                    "status": "completed",
                },
                duration_ms=10.0,
            )
