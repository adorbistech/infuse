"""Hermes Agent Adapter implementation of IUniversalAgentAdapter (Block 27)."""
...
class HermesAdapter(IUniversalAgentAdapter):
    """Universal agent adapter for Hermes Agent runtime (Block 27).

    Translates between the INFUSE universal agent contracts and Hermes CLI
    execution, tool activity, and control mechanics.
    """

    def __init__(
        self,
        agent_id: str = "hermes_adapter",
        agent_name: str = "hermes",
        version: str = "1.0.0",
        config: Optional[HermesAdapterConfig] = None,
        transport: Optional[IHermesTransport] = None,
        event_bus: Optional[IEventBus] = None,
    ) -> None:
        self._lock = threading.RLock()
        self._bus = event_bus
        self.config = config or HermesAdapterConfig()
        self.transport = transport or HermesSubprocessTransport(
        ...
        self._capability = AgentCapability(
            ...
            metadata={"vendor": "Nous Research Hermes Agent"},
        )
    ...
    def execute_step(self, request: AgentStepRequest) -> AgentStepResponse:
        ...
        if self.config.default_model:
            args.extend(["-m", self.config.default_model])
        ...
        try:
            ...
            raise AgentExecutionError(f"Unexpected Hermes execution failure: {e}") from e
        ...
        if self._bus is not None:
            try:
                ...
                ev = ExecutionEvent(
                    event_id=f"step_ev_hermes_{exec_id}_{request.step_index}_{int(now.timestamp()*1000)}",
                    execution_id=exec_id,
                ...
                if output.parsed_json and "tools" in output.parsed_json and isinstance(output.parsed_json["tools"], list):
                    for idx, tool in enumerate(output.parsed_json["tools"]):
                        ...
                        call_ev = ExecutionEvent(
                            event_id=f"tool_called_hermes_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
                        ...
                        comp_ev = ExecutionEvent(
                            event_id=f"tool_comp_hermes_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
                ...
                if output.parsed_json and "web_requests" in output.parsed_json and isinstance(output.parsed_json["web_requests"], list):
                    for idx, web in enumerate(output.parsed_json["web_requests"]):
                        w_url = str(web.get("url") or f"http://hermes.internal/{idx}")
                        req_ev = ExecutionEvent(
                            event_id=f"web_req_hermes_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
                        ...
                        resp_ev = ExecutionEvent(
                            event_id=f"web_resp_hermes_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
    ...
    def execute_control(self, operation: ControlOperation) -> ControlResult:
        ...
        if not is_supp:
            return ControlResult(
                ...
                message=f"Hermes adapter does not support control action '{action_name}'.",
                executed_at=now,
        ...
        try:
            ...
            return ControlResult(
                ...
                message=f"Hermes adapter successfully applied action '{action_name}'.",
                executed_at=now,
                metadata={"agent_id": self._identity.agent_id, "is_supported": True},
            )
        except Exception as e:
            redacted_err = redact_hermes_secrets(str(e))
            return ControlResult(
                operation_id=operation.operation_id,
                execution_id=exec_id,
                action=gov_action,
                status=ControlStatus.FAILED,
                message=f"Hermes adapter failed to execute control action '{action_name}': {redacted_err}",
                executed_at=now,
    ...
    def normalize_error(self, raw_error: Any, execution_id: Optional[str] = None) -> AgentErrorRecord:
        if isinstance(raw_error, Exception):
            ...
            code = str(raw_error.get("error_code") or raw_error.get("code") or "HERMES_ERROR")
            msg = str(raw_error.get("message") or raw_error.get("error") or "Unknown Hermes error")
        else:
            code = "HERMES_ERROR"
            msg = str(raw_error)
        ...
        return AgentErrorRecord(
            error_code=code,
            message=redact_hermes_secrets(msg),
