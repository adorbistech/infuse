"""Transport abstraction layer for OpenCode CLI and process management (Block 25)."""
...
class IOpenCodeTransport(ABC):
    """Abstract transport boundary between OpenCodeAdapter and OpenCode runtime."""

    @abstractmethod
    def is_available(self) -> bool:
        """Check whether the underlying OpenCode runtime is available."""
        pass
    ...
    def execute(
        ...
        """Execute an OpenCode CLI command with arguments."""
        pass
...
class OpenCodeSubprocessTransport(IOpenCodeTransport):
    """Subprocess-based transport communicating with local OpenCode CLI."""
    ...
    def execute(
        ...
        cli = self._cli_path or self._discover_cli()
        if not cli:
            raise OpenCodeCLINotFoundError("OpenCode executable ('opencode') not found on system PATH.")
        ...
        try:
            ...
            try:
                ...
                raise OpenCodeTimeoutError(
                    f"OpenCode process timed out after {exec_timeout} seconds for execution '{execution_id}'."
                )
            ...
            if proc.returncode != 0:
                redacted_err = redact_opencode_secrets(stderr)
                raise OpenCodeProcessError(
                    f"OpenCode process exited with return code {proc.returncode}: {redacted_err}"
                )

            return OpenCodeExecutionOutput(
                stdout=stdout or "",
                stderr=stderr or "",
...
class OpenCodeReferenceTransport(IOpenCodeTransport):
    """Deterministic reference/mock transport for hermetic testing of OpenCode adapter."""
    ...
    def __init__(self) -> None:
        ...
        self.custom_output: Optional[OpenCodeExecutionOutput] = None
    ...
    def execute(
        ...
        with self._lock:
            ...
            if self.should_raise_not_found:
                raise OpenCodeCLINotFoundError("OpenCode executable not found.")

            if self.should_timeout:
                raise OpenCodeTimeoutError("OpenCode reference execution timed out.")

            if self.should_fail:
                raise OpenCodeProcessError("OpenCode reference process exited with error.")
            ...
            return OpenCodeExecutionOutput(
                stdout=json.dumps({
                    "result": f"OpenCode response for prompt: {prompt_str[:40]}",
                    "status": "success",
                    "model": "default",
                }),
                stderr="",
