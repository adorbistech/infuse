"""OpenClaw Agent Adapter implementation of IUniversalAgentAdapter (Block 27)."""
...
class OpenClawAdapter(IUniversalAgentAdapter):
    """Universal agent adapter for OpenClaw runtime (Block 27).

    Translates between the INFUSE universal agent contracts and OpenClaw CLI
    execution, tool activity, and control mechanics.
    """

    def __init__(
        self,
        agent_id: str = "openclaw_adapter",
        agent_name: str = "openclaw",
        version: str = "1.0.0",
        config: Optional[OpenClawAdapterConfig] = None,
        transport: Optional[IOpenClawTransport] = None,
        event_bus: Optional[IEventBus] = None,
    ) -> None:
        self._lock = threading.RLock()
        self._bus = event_bus
        self.config = config or OpenClawAdapterConfig()
        self.transport = transport or OpenClawSubprocessTransport(
        ...
        self._capability = AgentCapability(
            ...
            metadata={"vendor": "OpenClaw AI Runtime"},
        )
    ...
    def execute_step(self, request: AgentStepRequest) -> AgentStepResponse:
        ...
        except OpenClawAdapterError as e:
            raise AgentExecutionError(str(e)) from e
        except Exception as e:
            raise AgentExecutionError(f"Unexpected OpenClaw execution failure: {e}") from e
        ...
        if self._bus is not None:
            try:
                ...
                ev = ExecutionEvent(
                    event_id=f"step_ev_openclaw_{exec_id}_{request.step_index}_{int(now.timestamp()*1000)}",
                    execution_id=exec_id,
                ...
                if output.parsed_json and "tools" in output.parsed_json and isinstance(output.parsed_json["tools"], list):
                    for idx, tool in enumerate(output.parsed_json["tools"]):
                        ...
                        call_ev = ExecutionEvent(
                            event_id=f"tool_called_openclaw_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
                        ...
                        comp_ev = ExecutionEvent(
                            event_id=f"tool_comp_openclaw_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
                ...
                if output.parsed_json and "web_requests" in output.parsed_json and isinstance(output.parsed_json["web_requests"], list):
                    for idx, web in enumerate(output.parsed_json["web_requests"]):
                        w_url = str(web.get("url") or f"http://openclaw.internal/{idx}")
                        req_ev = ExecutionEvent(
                            event_id=f"web_req_openclaw_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
                        ...
                        resp_ev = ExecutionEvent(
                            event_id=f"web_resp_openclaw_{exec_id}_{request.step_index}_{idx}",
                            execution_id=exec_id,
    ...
    def execute_control(self, operation: ControlOperation) -> ControlResult:
        ...
        if not is_supp:
            return ControlResult(
                ...
                message=f"OpenClaw adapter does not support control action '{action_name}'.",
                executed_at=now,
        ...
        try:
            ...
            return ControlResult(
                ...
                message=f"OpenClaw adapter successfully applied action '{action_name}'.",
                executed_at=now,
                metadata={"agent_id": self._identity.agent_id, "is_supported": True},
            )
        except Exception as e:
            redacted_err = redact_openclaw_secrets(str(e))
            return ControlResult(
                operation_id=operation.operation_id,
                execution_id=exec_id,
                action=gov_action,
                status=ControlStatus.FAILED,
                message=f"OpenClaw adapter failed to execute control action '{action_name}': {redacted_err}",
                executed_at=now,
    ...
    def normalize_error(self, raw_error: Any, execution_id: Optional[str] = None) -> AgentErrorRecord:
        if isinstance(raw_error, Exception):
            ...
            code = str(raw_error.get("error_code") or raw_error.get("code") or "OPENCLAW_ERROR")
            msg = str(raw_error.get("message") or raw_error.get("error") or "Unknown OpenClaw error")
        else:
            code = "OPENCLAW_ERROR"
            msg = str(raw_error)
        ...
        return AgentErrorRecord(
            error_code=code,
            message=redact_openclaw_secrets(msg),
